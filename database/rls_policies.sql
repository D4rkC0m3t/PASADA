...
-- Policies will be dropped and recreated if they already exist
--
-- =================================================================
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'clients') THEN
    ...
    -- Only admins can delete (archive) clients
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'projects') THEN
    ...
    -- Only admins and staff can create projects
    ...
    -- Only admins and staff can update projects
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'materials') THEN
    ...
    -- Everyone (authenticated) can view materials (catalog)
    CREATE POLICY "Anyone can view materials"
      ON materials FOR SELECT
      USING (auth.role() = 'authenticated');

    -- Only admins and staff can create materials
    ...
    -- Only admins and staff can update materials
...
-- Clients can only view their own quotations
...
-- Only admins and staff can create quotations
...
-- Only admins and staff can update quotations
...
CREATE POLICY "Clients can update quotation status"
  ...
  WITH CHECK (
    -- Clients can only update status field (all other fields must remain the same)
    OLD.id = NEW.id AND
...
-- Inherit quotation policies (quotation_items belong to quotations)
...
-- Clients can view quotation items for their quotations
...
-- Only admins and staff can manage quotation items
...
-- Anyone can create a booking (new clients)
CREATE POLICY "Anyone can create bookings"
  ON bookings FOR INSERT
  WITH CHECK (true);

-- Only admins and staff can update bookings
...
CREATE POLICY "Clients can cancel own bookings"
  ...
  WITH CHECK (
    -- Clients can only update status to 'cancelled'
...
CREATE OR REPLACE FUNCTION owns_client(client_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM clients
